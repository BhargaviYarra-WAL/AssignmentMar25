...
router.get("/setcookiewithtime/:name/:value/:time", function (req, res) {
  ...
  res.send(
    `cookie with name ${req.params.name} and value ${req.params.value} will expire in ${req.params.time} mins`
  );
});
...
});
{
  ...
  res.send(
    `cookie with city ${req.params.city}} and value ${req.params.value}`
  );
});*/
}
